...
const initCommand = (baseDir, logger) => {
  ...
  const removeForcibly = async (firstPath, ...restPath) => {
    const path = currentPath(firstPath, ...restPath);
    await rm(path, { force: true, recursive: true });
    logger(`=> ${emphasize(relative(baseDir, path))} was removed`);
  };
  ...
  return {
    async updatePackageFile() {
      ...
      for (const [key, script] of Object.entries(originalPackage.scripts)) {
        if (!(key === "test" || key.startsWith("test:") || key === "postprepare")) {
          scripts[key] = script;
        }
      }
      ...
      // Use flat config file instead of legacy package.json "eslintConfig".
      // Remove any existing legacy config to avoid confusion.
      delete packageInfo.eslintConfig;
     ...
     */
    async copyPackageFile(from, to) {
      await copy(packagePath(...from), currentPath(...to));
    },
    ...
    async removeNeedlessFiles() {
      ...
      // husky is no longer used.
      await removeForcibly(".husky");

      // The `.remarkignore` used only to ignore `CHANGELOG.md` is no longer needed,
      // because `CHANGELOG.md` now disables the relevant rules inline.
      // Remove it only when it contains nothing else.
      const remarkIgnore = await read(".remarkignore").catch((err) => {
        if (err.code === "ENOENT") return null;
        throw err;
      });
      if (remarkIgnore !== null) {
        const entries = remarkIgnore
          .split("\n")
          .map((line) => line.trim())
          .filter(Boolean);
        if (entries.length === 1 && entries[0] === "CHANGELOG.md") {
          await removeForcibly(".remarkignore");
        }
      }
    },
  };
...
export async function init({ cwd = process.cwd(), logger = defaultLogger } = {}) {
  ...
  await cmd.copyPackageFile(
    ["lib", "templates", "eslint.config.template.js"],
    ["eslint.config.js"],
  );
  await cmd.writePackageFile(".githooks", "commit-msg");
  await cmd.writePackageFile(".githooks", "pre-commit");
  ...
  await cmd.writePackageFile(".github", "workflows", "release-pr.yml");
