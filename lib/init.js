...
const initCommand = (baseDir, logger) => {
   ...
   */
  const exist = (fileName) => existsSync(currentPath(fileName));
  ...
  return {
    ...
    async removeNeedlessFiles() {
      ...
      // If `.remarkignore` contains only `CHANGELOG.md`, it's removed. `CHANGELOG.md` is now linted.
      const remarkIgnoreFile = ".remarkignore";
      if (exist(remarkIgnoreFile)) {
        const remarkIgnore = await read(remarkIgnoreFile);
        if (remarkIgnore.trim() === "CHANGELOG.md") {
          await removeForcibly(remarkIgnoreFile);
        }
      }
    },
  };
